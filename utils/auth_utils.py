...
def handle_verify_email_action(oob_code: str):
    """
    STEP 1: Validate the verification link from email.
    Returns success with uid if link is valid, but does NOT set verified status yet.
    """
    try:
        api_key = firebaseConfig["apiKey"]
        url = f"https://identitytoolkit.googleapis.com/v1/accounts:update?key={api_key}"
        ...
        # CRITICAL: DO NOT update verification status yet
        # Just return that the link is valid
        print(f"Verification link validated for UID: {uid}, Email: {email}")
        return True, "Verification link is valid.", email, uid
        ...
        if e.response is not None:
            try:
                ...
                if "INVALID_OOB_CODE" in error_message:
                    return False, "This verification link is invalid or has already been used.", None, None
                elif "EXPIRED_OOB_CODE" in error_message:
                    return False, "This verification link has expired. Please request a new one.", None, None
                else:
                    return False, "Unable to verify email. Please try again or request a new verification email.", None, None
            except:
                pass
        ...
        print(f"Error handling email verification: {e}")
        return False, "An error occurred while verifying your email. Please try again.", None, None

def complete_email_verification(uid: str, oob_code: str):
    """
    STEP 2: Actually mark the user as verified after they click the confirmation button.
    This is called when user clicks "Confirm My Email" button.
    """
    try:
        user_ref = db.child("users").child(uid)
        
        # Set verified status in database
        user_ref.update({
            "email_verified": True,
            "verification_token": oob_code,
            "verified_at": datetime.datetime.now(datetime.timezone.utc).isoformat()
        })
        
        print(f"✅ Email verified for UID: {uid}")
        return True, "Email verified successfully! You can now log in."
        
    except Exception as e:
        print(f"❌ Error completing verification: {e}")
        return False, "Failed to complete verification. Please try again."
...
def create_account(email: str, password: str, first_name: str, last_name: str):
    ...
    try:
        ...
        # Save profile with verification LOCKED at False
        user_ref = db.child("users").child(uid)
        user_ref.set({
            "email": email,
            "name": f"{first_name} {last_name}".strip(),
            "role": role,
            "created_at": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "email_verified": False  # Will ONLY change when user clicks confirm button
        })
...
def sign_in(email: str, password: str):
    """
    Sign in user and check verification status from database only.
    Ignores Firebase Auth's emailVerified field completely.
    """
    api_key = firebaseConfig["apiKey"]
    url = f"https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key={api_key}"
    ...
    # Get verification status from OUR database (ignore Firebase Auth)
    user_ref = db.child("users").child(uid)
    profile = user_ref.get() or {}
    email_verified = profile.get("email_verified", False)
    
    print(f"Sign-in - UID: {uid}, Database email_verified: {email_verified}")
