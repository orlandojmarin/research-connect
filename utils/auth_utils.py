...
def send_verification_email(id_token: str):
    ...
    try:
        ...
        print(f"Timeout sending verification email - Firebase may be slow")
        return False
    except Exception as e:
        print(f"Error sending verification email: {e}")
        return False
...
def create_account(email: str, password: str, first_name: str, last_name: str):
    """
    Create a new Firebase user, send verification email, and store their profile.
    Returns (uid, id_token, email_sent_successfully)
    """
    # Create user account using REST API
    api_key = firebaseConfig["apiKey"]
    url = f"https://identitytoolkit.googleapis.com/v1/accounts:signUp?key={api_key}"
    ...
    # Save basic profile in DB (this is fast)
    user_ref = db.child("users").child(uid)
    user_ref.set({
        "email": email,
        "name": f"{first_name} {last_name}".strip(),
    ...
    # Note: This can be slow, but we don't want to fail account creation if email fails
    email_sent = False
    try:
        email_sent = send_verification_email(id_token)
    except Exception as e:
        print(f"Warning: Failed to send verification email immediately: {e}")
        # Account is still created successfully, email just didn't send
        # User can resend it later from the verification page

    return uid, id_token, email_sent
...
def sign_in(email: str, password: str):
    ...
    print(f"DEBUG sign_in: uid={uid}, email_verified from Admin SDK={email_verified}")
    
    # CRITICAL: Only update database if email is actually verified
    # Do NOT update to True unless Firebase Auth confirms it
    if email_verified:
        user_ref = db.child("users").child(uid)
        user_ref.update({"email_verified": True})
        print(f"DEBUG: Updated database - email_verified=True for uid={uid}")
    else:
        # Ensure database shows unverified status
        user_ref = db.child("users").child(uid)
        current_status = user_ref.child("email_verified").get()
        print(f"DEBUG: Database shows email_verified={current_status} for uid={uid}")
        
        # If database incorrectly shows verified, fix it
        if current_status is True:
            print(f"WARNING: Database incorrectly showed verified, correcting to False")
