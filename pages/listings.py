...
#     with tab1:
#         listings = get_all_listings_from_firebase()[::-1]  # Firebase only
#         hours_filter, compensation_filter, faculty_filter = render_sidebar_filters(listings)
#         filtered_listings = filter_listings(listings, hours_filter, compensation_filter, faculty_filter)
...
# TATIANA
# listings.py
# hourly pay rate field always visible (grayed out depending on compensation type)
...
#     # Remove st.form to allow dynamic updates
#     title = st.text_input("Project Title *", value=listing['title'], key=f"edit_title_{form_key_prefix}_{listing_id}")
#     team = st.text_input("Additional Collaborators", value=listing.get('team', 'n/a') if listing.get('team') != 'n/a' else "", key=f"edit_team_{form_key_prefix}_{listing_id}")
...
#     col1, col2 = st.columns(2)
#     with col1:
#         save_button = st.button("💾 Save Changes", key=f"save_{form_key_prefix}_{listing_id}", use_container_width=True)
#     with col2:
#         cancel_button = st.button("❌ Cancel", key=f"cancel_{form_key_prefix}_{listing_id}", use_container_width=True)
...
# TATIANA
# listings.py
# compensation type PAID selected by default
...
#     # Initialize session state for compensation type if not exists
#     comp_session_key = f"edit_comp_{form_key_prefix}_{listing_id}"
#     if comp_session_key not in st.session_state:
#         st.session_state[comp_session_key] = "Paid" if listing['compensation_type'] == "paid" else "Unpaid"
...
#         # Default to Paid
#         comp_index = 0 if st.session_state[comp_session_key] == "Paid" else 1
#         compensation_type = st.radio("Compensation Type *", ["Paid", "Unpaid"], index=comp_index, key=f"edit_comp_type_{form_key_prefix}_{listing_id}")
        
#         # Always show pay rate field, disabled when Unpaid
#         pay_rate = st.number_input(
#             "Hourly Pay Rate ($) *",
#             min_value=16.35,
#             value=max(16.35, float(listing.get('pay_rate', 16.35))),
#             step=0.01,
#             format="%.2f",
#             disabled=(st.session_state[comp_session_key] == "Unpaid"),
#             key=f"edit_pay_rate_{form_key_prefix}_{listing_id}",
#             help="This field is disabled for unpaid positions" if st.session_state[comp_session_key] == "Unpaid" else None
#         )
...
#         if cancel_button:
#             st.session_state.editing_listing = None
#             if comp_session_key in st.session_state:
#                 del st.session_state[comp_session_key]
#             st.rerun()
        
#         if save_button:
#             # Update session state with current selection
#             if compensation_type != st.session_state[comp_session_key]:
#                 st.session_state[comp_session_key] = compensation_type
#                 st.rerun()  # Rerun to update the disabled state
...
#                     compensation_type = st.radio("Compensation Type *", ["Paid", "Unpaid"], index=0, key=f"comp_type_{form_key}")
                    
#                     # Always show pay rate field, disabled when Unpaid is selected, default to Paid
#                     pay_rate = st.number_input(
#                         "Hourly Pay Rate ($) *",
#                         min_value=16.35,
#                         value=16.35,
...
#                         disabled=(compensation_type == "Unpaid"),
#                         key=f"pay_rate_input_{form_key}",
#                         help="This field is disabled for unpaid positions" if compensation_type == "Unpaid" else None
#                     )
